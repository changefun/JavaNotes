...
- __c__  方法区内，发现main主方法时，将main()压进栈内存区。
- __d__  main()进栈，执行main()里的语句，发现new Student()语句时执行.
- __e__ 执行到new Student()时，类加载器将Student.class文件载入JVM内存方法区中。
- __f__ 然后在堆内存中开辟新的空间用来存储Student对象的属性，并初始化这些属性（初始化分为默认初始化和构造函数初始化）。
